// RecordDemoDlg.cpp : implementation file
//

#include "stdafx.h"
#include "RecordDemo.h"
#include "RecordDemoDlg.h"
...
// CRecordDemoDlg dialog
CRecordDemoDlg::CRecordDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CRecordDemoDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CRecordDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST1, m_EventListCtrl);
}

BEGIN_MESSAGE_MAP(CRecordDemoDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BUTTON_ENABLE, &CRecordDemoDlg::OnBnClickedButtonEnable)
	ON_BN_CLICKED(IDC_BUTTON_DISABLE, &CRecordDemoDlg::OnBnClickedButtonDisable)
	ON_BN_CLICKED(IDC_BUTTON_CLEAR, &CRecordDemoDlg::OnBnClickedButtonClear)
END_MESSAGE_MAP()


// CRecordDemoDlg message handlers

BOOL CRecordDemoDlg::OnInitDialog()
{
...
void CRecordDemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CRecordDemoDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CRecordDemoDlg::OnBnClickedButtonEnable()
{
	UsbBox_EnableDevice();
	UsbBox_EventCallBack(USBCallBackEvent);							 
	UsbBox_EnableLog(1);

	GetDlgItem(IDC_BUTTON_ENABLE)->EnableWindow(FALSE);
	GetDlgItem(IDC_BUTTON_DISABLE)->EnableWindow(TRUE);

	UsbBox_SetDeviceType(USBBOX_TYPE_F1);			// set device type  AD102
	UsbBox_SetRecordFormat(USBBOX_RECORDFORMAT_MP3);		// set wave record
	UsbBox_SetRecordFolder("D:\\");							// set record folder

	UsbBox_EnableRecord(0,1);   // ENABLE RECORD
	UsbBox_SetAutoRecord(0,1);  // SET AUTO RECORD 
}

void CRecordDemoDlg::OnBnClickedButtonDisable()
{
...
void CALLBACK  CRecordDemoDlg::USBCallBackEvent(WORD wEventCode, int nReference,DWORD dwParam,DWORD dwDeviceID)
{
	switch ( wEventCode )
	{
	case EVENT_STATE:
...
void CRecordDemoDlg::OnBnClickedButtonClear()
{
